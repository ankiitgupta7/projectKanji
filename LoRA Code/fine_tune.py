...
class KanjiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt = entry["meanings"][0] if entry["meanings"] else "Kanji"  # Default to "Kanji" if empty
...
vae = pipeline.vae.to("cuda" if torch.cuda.is_available() else "cpu")  # Move VAE to GPU
text_encoder = pipeline.text_encoder.to("cuda" if torch.cuda.is_available() else "cpu")  # Move text encoder to GPU
